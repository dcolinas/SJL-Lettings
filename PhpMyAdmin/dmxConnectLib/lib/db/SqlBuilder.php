...
class SqlBuilder
{
    ...
    public function fromJSON($json) {
		...
		$props = array('type', 'table', 'joins', 'values', 'wheres', 'orders', 'columns', 'params', 'sortables', 'offset', 'limit', 'distinct');
        ...
        array_walk(array(
            'type', 'table', 'joins', 'values', 'wheres', 'orders', 'columns', 'params', 'sortables'
        ), function($prop) use ($self, $json) {
            if (isset($json->$prop)) {
                $self->$prop = $json->$prop;
            }
        });
    ...
    private function compileJoin($join) {
        return strtoupper($join->type) . ' JOIN ' . $this->quoteIdentifier($join->table) . ' ON ' . preg_replace('/^(AND|OR)\s*/i', '', implode(' ', array_map(array($this, 'compileWhere'), $join->clauses)));
    }
